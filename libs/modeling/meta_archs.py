...
class PtTransformer(nn.Module):
    ...
    """
    print("Enter LocPointTransformer class")
    def __init__(
        self,
    ...
    def forward(self, video_list):
        print("Enter LocPointTransformer Forword Function")
        # batch the video list into feats (B, C, T) and masks (B, 1, T)
        batched_inputs, batched_masks = self.preprocessing(video_list)
        print("After 1st line")
    
        # Print batched inputs and masks
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        if self.training:
            ...
            # Create a tensor with the desired shape and padding
            padding_size = (0, max_len - feats_lens[0])  # Pad the last dimension
            batched_inputs = F.pad(
                feats[0], padding_size, value=padding_val).unsqueeze(0)

        # Ensure the values are in the expected range (if necessary)
        batched_inputs = torch.clamp(batched_inputs, min=0, max=1)  # Adjust based on your model's requirements
        ...
        if batched_inputs.dim() == 4:  # If it has an extra dimension
            batched_inputs = batched_inputs.squeeze(1)  # Remove the second dimension
