...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        for idx in range(arch[0]):
            in_ch = n_embd if idx > 0 else n_in
            self.embd.append(
                MaskedConv1D(
        ...
        self.se_block = nn.Sequential(
            ...
            nn.Conv1d(n_embd, n_embd // 8, kernel_size=1),
            nn.GELU(),
            nn.Conv1d(n_embd // 8, n_embd, kernel_size=1),
            nn.Sigmoid()
        )

        # absolute position embedding
        if self.use_abs_pe:
            self.pos_embd = nn.Parameter(torch.randn(1, n_embd, self.max_len))
    ...
    def forward(self, x, mask):
        ...
        # embedding network with residual connection
        for idx in range(len(self.embd)):
            residual = x
            x, mask = self.embd[idx](x, mask)
            x = self.act(self.embd_norm[idx](x))
            if residual.shape == x.shape:
                x = x + residual  # residual connection

        # Squeeze-and-Excitation (channel-wise reweighting)
